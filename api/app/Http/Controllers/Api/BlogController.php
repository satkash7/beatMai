...
class BlogController extends Controller
{
   ...
   public function updateBlog(Request $request, $id) {
    ...
    /**
     * Get blog URLs for sitemap generation
     * Returns an array of blog routes for the Nuxt sitemap module
     */
    public function getSitemapUrls() {
        try {
            // Get all published blogs
            $blogs = Blog::select('blogRoute', 'created_at', 'updated_at')
                        ->orderBy('created_at', 'desc')
                        ->get();
            
            // Extract just the routes as a simple array
            $blogRoutes = $blogs->map(function ($blog) {
                return $blog->blogRoute;
            })->toArray();
            
            return response()->json([
                'status_code' => 200,
                'status_message' => 'Blog sitemap URLs retrieved successfully',
                'urls' => $blogRoutes,
                'count' => count($blogRoutes),
            ...
            return response()->json([
                'status_code' => 500,
                'status_message' => 'Error generating sitemap URLs',
                'urls' => [],
                'error' => $e->getMessage()
            ], 500);
        }
