  ...
  <div class="flex items-center space-x-4">
    ...
    <div class="relative w-full max-w-[106px] sm:max-w-[159px]">
      <button
        ...
        @blur="toggleDropdown"
      >
        <img :src="getExchangeImg(exchangeSelected.img)" alt="" class="flex-shrink-0 h-6 w-6 rounded-full" />
        ...
        >
          ...
          >
            <div class="flex items-center">
              <img :src="getExchangeImg(exchange.img)" alt="" class="flex-shrink-0 h-6 w-6 rounded-full" />
...
<script>
const exchangeImages = import.meta.glob('~/assets/img/*', { eager: true, import: 'default' })
...
export default {
  ...
  methods: {
    ...
    getExchangeImg(filename) {
      const key = `/assets/img/${filename}`
      for (const [path, mod] of Object.entries(exchangeImages)) {
        if (path.endsWith(filename)) return mod
      }
      return ''
    }
  }
